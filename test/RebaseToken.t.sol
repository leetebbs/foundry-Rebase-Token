...
contract RebaseTokenTest is Test {
    ...
    function setUp() public {
        ...
        rebaseToken.grantBurnAdMintRole(address(vault));
        vm.stopPrank();
    }
    ...
    function testGetRebaseTokenAddress() public view{
        // Get the address of the RebaseToken contract from the Vault
        address rebaseTokenAddress = vault.getRebaseTokenAddress();
        // Check that the address is the same as the one we set in the constructor
    ...
    function testTransferFrom() public {
        ...
        // User approves userTwo to spend their tokens
        vm.prank(user);
        rebaseToken.approve(userTwo, amount);
        assertEq(rebaseToken.allowance(user, userTwo), amount);
        
        // UserTwo calls transferFrom to transfer from user to themselves
        vm.prank(userTwo);
        rebaseToken.transferFrom(user, userTwo, amount);
        
        assertEq(rebaseToken.balanceOf(user), 0);
        assertEq(rebaseToken.balanceOf(userTwo), amount);    
    }
