...
contract CrossChainTest is Test {
    ...
    function setUp() public {
        ...
        // Configure cross-chain token pools after both are deployed
        configureTokenPool(
            sepoliaFork,
            address(sepoliaPool),
    ...
    function testBridgePartialTokens() public {
        ...
        uint256 bridgeAmount = SEND_VALUE / 2;
        bridgeTokens(bridgeAmount, sepoliaFork, arbSepoliaFork, sepoliaNetworkDetails, arbSepoliaNetworkDetails, sepoliaToken, arbSepoliaToken);
        
        // Check remaining balance on source chain
        assertEq(sepoliaToken.balanceOf(user), SEND_VALUE - bridgeAmount);
    }
    ...
    function testBridgeBackAndForth() public {
        ...
        // Bridge from Sepolia to Arbitrum
        bridgeTokens(SEND_VALUE, sepoliaFork, arbSepoliaFork, sepoliaNetworkDetails, arbSepoliaNetworkDetails, sepoliaToken, arbSepoliaToken);
        
        // Bridge back from Arbitrum to Sepolia
        bridgeTokens(SEND_VALUE, arbSepoliaFork, sepoliaFork, arbSepoliaNetworkDetails, sepoliaNetworkDetails, arbSepoliaToken, sepoliaToken);
    ...
    function testVaultDeposit() public {
        ...
        assertEq(sepoliaToken.balanceOf(user), SEND_VALUE);
        assertEq(address(vault).balance, SEND_VALUE);
    }

    function testTokenPoolConfiguration() public {
        vm.selectFork(sepoliaFork);
        assertTrue(address(sepoliaPool) != address(0));
        assertTrue(sepoliaPool.isSupportedToken(address(sepoliaToken)));
        
        vm.selectFork(arbSepoliaFork);
        assertTrue(address(arbSepoliaPool) != address(0));
        assertTrue(arbSepoliaPool.isSupportedToken(address(arbSepoliaToken)));
    }
}